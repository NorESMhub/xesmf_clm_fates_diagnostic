...
def make_regular_grid_regridder(regrid_start, regrid_target, method= "bilinear"):
    ...
    lon_min = np.argmin(np.abs(regrid_target["lon"].values - regrid_start["lon"].values.min()))
    lon_max = np.argmin(np.abs(regrid_target["lon"].values - regrid_start["lon"].values.max()))
    regrid_target = regrid_target.isel(lat=slice(lat_min, lat_max), lon=slice(lon_min,lon_max))
